...
class Listener:
    def __init__(self):
        self.w3 = getNode()
        self.contract = getContract(self.w3, ONE_THOUSAND_CARDS_ADDRESS, ONE_THOUSAND_CARDS_INSTANCE)
        self.eggLaidFilter = self.contract.events.crateLaid.createFilter(fromBlock='latest')
        self.cardCreatedFilter = self.contract.events.cardCreated.createFilter(fromBlock='latest')
        self.loop = asyncio.get_event_loop()
        self.initialised = False
        try:
            self.loop.run_until_complete(asyncio.gather(self.eggLaidLoop(2), self.cardCreatedLoop(2), self.checkServer(10)))
        finally:
            self.loop.close()
    ...
    def startUp(self):
        ...
        state = create_DB()
        eggEvents = list(fetchEvents(self.contract.events.crateLaid, from_block=0))
